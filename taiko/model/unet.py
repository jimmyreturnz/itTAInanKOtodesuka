...
class TaikoDiffusionUNet(nn.Module):
    def __init__(
        ...
        for lvl, mult in enumerate(channel_mult):
            ...
            self.enc_attn.append(CrossAttention1D(out_ch, audio_channels, n_heads=max(1, out_ch // 64)))
        ...
        self.mid_attn   = CrossAttention1D(in_ch, audio_channels, n_heads=max(1, in_ch // 64))
        ...
        for lvl, mult in enumerate(reversed(channel_mult)):
            ...
            res_list = nn.ModuleList()
            cur_in   = in_ch + skip_ch
            for i in range(num_res_blocks):
                res_list.append(ResBlock(cur_in, out_ch, emb_dim, num_groups, dropout))
                cur_in = out_ch
            self.dec_res.append(res_list)
            self.dec_attn.append(CrossAttention1D(out_ch, audio_channels, n_heads=max(1, out_ch // 64)))
    ...
    def forward(
        ...
        for lvl in range(self.num_levels):
            for blk in self.enc_res[lvl]:
                h = blk(h, emb)
            af = self._interpolate_audio(audio_features, h.shape[2])
