"""
taiko/model/diffusion.py

Updated with avg_nps + peak_nps conditioning.
Profile-driven init via model_config.DiffusionProfile.
"""

from __future__ import annotations

import torch
import torch.nn as nn
import torch.nn.functional as F

from taiko.model.autoencoder import BeatmapAutoencoder, AutoencoderConfig
from taiko.model.audio_encoder import MelEncoder1D
from taiko.model.unet import TaikoDiffusionUNet
from taiko.model.noise_scheduler import NoiseScheduler
from taiko.model.model_config import DiffusionProfile, get_profile

MOTIF_DIM = 16


def disabled_train(self, mode=True):
    return self


class TaikoDiffusion(nn.Module):

    def __init__(
        self,
        autoencoder_ckpt: str,
        profile: str | DiffusionProfile = "p1",
        timesteps: int = 1000,
        beta_schedule: str = "linear",
        cfg_scale: float = 1.5,
        z_channels: int = 16,
        n_mels: int = 128,
        n_styles: int = 4,
    ):
        super().__init__()

        self.cfg_scale = cfg_scale
        self.z_channels = z_channels

        if isinstance(profile, str):
            profile = get_profile(profile)

        self.profile = profile

        # ------------------------------------------------------------------
        # Autoencoder (Frozen)
        # ------------------------------------------------------------------

        ae_config = AutoencoderConfig(z_channels=z_channels)

        self.first_stage_model = BeatmapAutoencoder(ae_config)

        ckpt = torch.load(autoencoder_ckpt, map_location="cpu")

        self.first_stage_model.load_state_dict(ckpt["model"])

        self.first_stage_model.eval()

        self.first_stage_model.train = disabled_train.__get__(
            self.first_stage_model
        )

        for p in self.first_stage_model.parameters():
            p.requires_grad = False

        print(
            f"Loaded frozen autoencoder from "
            f"{autoencoder_ckpt} (step {ckpt['step']})"
        )

        # ------------------------------------------------------------------
        # Audio encoder
        # ------------------------------------------------------------------

        self.wave_model = MelEncoder1D(
            n_mels=n_mels,
            base_channels=profile.audio_base_channels,
            channel_mult=list(profile.audio_channel_mult),
        )

        print(
            f"Audio encoder: "
            f"{len(profile.audio_channel_mult)} levels, "
            f"out={profile.audio_out_channels}"
        )

        # ------------------------------------------------------------------
        # U-Net
        # ------------------------------------------------------------------

        self.unet_model = TaikoDiffusionUNet(
            z_channels=z_channels,
            base_channels=profile.unet_base_channels,
            channel_mult=list(profile.unet_channel_mult),
            num_res_blocks=profile.unet_num_res_blocks,
            n_styles=n_styles,
            audio_channels=profile.unet_audio_channels,
            cfg_dropout=profile.cfg_dropout,
            use_checkpoint=profile.use_checkpoint,
            use_s4=profile.use_s4,
            use_motif=profile.use_motif,
            use_nps=profile.use_nps,
        )

        unet_p = sum(
            p.numel() for p in self.unet_model.parameters()
        ) / 1e6

        wave_p = sum(
            p.numel() for p in self.wave_model.parameters()
        ) / 1e6

        print(f"U-Net: {unet_p:.1f}M | Audio enc: {wave_p:.1f}M")

        # ------------------------------------------------------------------
        # Noise scheduler
        # ------------------------------------------------------------------

        self.scheduler = NoiseScheduler(
            timesteps=timesteps,
            beta_schedule=beta_schedule,
        )

    def to(self, device):
        super().to(device)
        self.scheduler.to(device)
        return self

    # ----------------------------------------------------------------------
    # Encode / Decode
    # ----------------------------------------------------------------------

    def encode(self, beatmap_tensor):

        with torch.no_grad():
            return self.first_stage_model.encode(
                beatmap_tensor
            ).mode()

    def decode(self, z):

        with torch.no_grad():
            return torch.sigmoid(
                self.first_stage_model.decode(z)
            )

    # ----------------------------------------------------------------------
    # Training loss
    # ----------------------------------------------------------------------

    def training_loss(
        self,
        beatmap: torch.Tensor,
        mel: torch.Tensor,
        difficulty: torch.Tensor,
        style: torch.Tensor,
        valid_mask: torch.Tensor,
        avg_nps: torch.Tensor | None = None,
        peak_nps: torch.Tensor | None = None,
        motif: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, dict]:

        B = beatmap.shape[0]
        device = beatmap.device

        # Encode beatmap → latent
        z = self.encode(beatmap)

        # Sample timestep
        t = torch.randint(
            0,
            self.scheduler.timesteps,
            (B,),
            device=device,
        ).long()

        # Add noise
        noise = torch.randn_like(z)

        z_noisy = self.scheduler.add_noise(
            z,
            t,
            noise,
        )

        # Audio features
        audio_features = self.wave_model(mel)

        # Predict noise
        noise_pred = self.unet_model(
            z_noisy,
            t,
            audio_features,
            difficulty,
            style,
            avg_nps=avg_nps if self.profile.use_nps else None,
            peak_nps=peak_nps if self.profile.use_nps else None,
            motif=motif if self.profile.use_motif else None,
        )

        # ------------------------------------------------------------------
        # Masking
        # ------------------------------------------------------------------

        T_lat = z.shape[2]

        mask = (
            valid_mask[:, ::16][:, :T_lat]
            .unsqueeze(1)
        )

        # ------------------------------------------------------------------
        # Loss
        # ------------------------------------------------------------------

        loss_per_element = F.smooth_l1_loss(
            noise_pred,
            noise,
            reduction="none",
        )

        loss = (
            (loss_per_element * mask).sum()
            / mask.sum().clamp(min=1)
        )

        with torch.no_grad():

            mae = (
                ((noise_pred - noise).abs() * mask).sum()
                / mask.sum().clamp(min=1)
            )

        return loss, {
            "loss": loss.item(),
            "mae": mae.item(),
            "t_mean": t.float().mean().item(),
            "mask_ratio": mask.mean().item(),
        }

    # ----------------------------------------------------------------------
    # Generation
    # ----------------------------------------------------------------------

    @torch.no_grad()
    def generate(
        self,
        mel: torch.Tensor,
        difficulty: float,
        style: int,
        latent_length: int,
        avg_nps: float | None = None,
        peak_nps: float | None = None,
        motif: list[float] | None = None,
        ddim_steps: int = 50,
        cfg_scale: float = None,
        eta: float = 0.0,
        device: torch.device = None,
    ) -> torch.Tensor:

        if device is None:
            device = next(self.parameters()).device

        if cfg_scale is None:
            cfg_scale = self.cfg_scale

        B = 1

        diff_t = torch.tensor(
            [difficulty],
            device=device,
        )

        style_t = torch.tensor(
            [style],
            device=device,
            dtype=torch.long,
        )

        anps_t = (
            torch.tensor([avg_nps], device=device)
            if avg_nps is not None
            else None
        )

        pnps_t = (
            torch.tensor([peak_nps], device=device)
            if peak_nps is not None
            else None
        )

        motif_t = (
            torch.tensor(
                [motif],
                dtype=torch.float32,
                device=device,
            )
            if motif is not None
            else None
        )

        # Audio features
        audio_features = self.wave_model(mel)

        # Initial noise
        z = torch.randn(
            B,
            self.z_channels,
            latent_length,
            device=device,
        )

        # DDIM schedule
        step_size = self.scheduler.timesteps // ddim_steps

        timesteps = list(
            reversed(
                range(
                    0,
                    self.scheduler.timesteps,
                    step_size,
                )
            )
        )

        # ------------------------------------------------------------------
        # Sampling loop
        # ------------------------------------------------------------------

        for i, t_val in enumerate(timesteps):

            t_prev_val = (
                timesteps[i + 1]
                if i + 1 < len(timesteps)
                else 0
            )

            t = torch.full(
                (B,),
                t_val,
                device=device,
                dtype=torch.long,
            )

            t_prev = torch.full(
                (B,),
                t_prev_val,
                device=device,
                dtype=torch.long,
            )

            # Conditional prediction
            noise_cond = self.unet_model(
                z,
                t,
                audio_features,
                diff_t,
                style_t,
                avg_nps=anps_t,
                peak_nps=pnps_t,
                motif=motif_t,
                drop_cond=False,
            )

            # CFG
            if cfg_scale != 1.0:

                noise_uncond = self.unet_model(
                    z,
                    t,
                    audio_features,
                    torch.zeros_like(diff_t),
                    torch.zeros_like(style_t),
                    avg_nps=(
                        torch.zeros_like(anps_t)
                        if anps_t is not None
                        else None
                    ),
                    peak_nps=(
                        torch.zeros_like(pnps_t)
                        if pnps_t is not None
                        else None
                    ),
                    motif=(
                        torch.zeros_like(motif_t)
                        if motif_t is not None
                        else None
                    ),
                    drop_cond=True,
                )

                noise_pred = (
                    noise_uncond
                    + cfg_scale
                    * (noise_cond - noise_uncond)
                )

            else:
                noise_pred = noise_cond

            # DDIM step
            z = self.scheduler.ddim_sample(
                noise_pred,
                z,
                t,
                t_prev,
                eta=eta,
            )

        return z