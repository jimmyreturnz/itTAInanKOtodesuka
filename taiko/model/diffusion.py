...
class TaikoDiffusion(nn.Module):
    ...
    def training_loss(
        ...
        z_noisy, noise = self.scheduler.add_noise(z, t, noise)

        # Get audio features
        audio_features = self.wave_model(mel)             # list of multi-scale tensors
        ...
        # MSE loss on noise prediction (eps parameterization like Mug-Diffusion)
        loss = nn.functional.mse_loss(noise_pred, noise)

        # Additional MAE for logging
        with torch.no_grad():
            mae = (noise_pred - noise).abs().mean()
